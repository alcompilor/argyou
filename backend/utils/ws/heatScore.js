...
export function measureHeat(newMessage) {
  ...
  const messageDefaultTime = new Date(newMessage.publishDate);
  messageDefaultTime.setHours(messageDefaultTime.getHours() - 2);
  const messageTime = messageDefaultTime.getTime();
  ...
  const frequencyRate =
    messageCount / ((currentTime - lastUpdateTime) / 1000 / 60);
