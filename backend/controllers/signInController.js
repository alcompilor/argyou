...
export async function signIn(req, res, next) {
    try {
        ...
        if (!user) {
            return res.status(404).json(new ResponseData("Authentication failed. User not found.", 404));
        }

        const match = await bcrypt.compare(password, user.password);
        if (!match) {
            return res.status(401).json(new ResponseData("Authentication failed. Invalid password.", 401));
        }
